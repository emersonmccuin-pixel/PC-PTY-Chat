...
async function registerChild(projectId: ULID, slug: string): Promise<RegisteredChild> {
  // Section 18.5a — `/channel-register` requires sessionId. For the
  // workflow-firing-smoke test the channel event is generated by the
  // workflow runtime (server-side) and doesn't address a specific
  // orchestrator session; the test registers a stable fake sessionId
  // and the runtime's emit path uses `emitToSession` with the same id.
  const sessionId = `smoke-${projectId}`;
  const url = `ws://127.0.0.1:${channelPort}/channel-register?projectId=${projectId}&sessionId=${encodeURIComponent(sessionId)}&slug=${slug}`;
