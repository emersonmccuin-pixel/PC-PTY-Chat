...
// ── 4f.5 / D39 lifecycle + manual-fire + validation HTTP smoke ──────────────
//
// Drives every 4f route exercised by the UI through `app.fetch(new Request(...))`
// against a focused Hono mirror of the production endpoints. Catches wire-layer
// regressions the unit tests miss (URL drift, JSON-shape changes, error-code
// mappings). One comprehensive walk per arc:
//
//   1. Create → Edit (PUT def) → Edit (PUT yamlText) → confirm persisted
//   2. Delete with in-flight run → 409 → cancel-runs-and-delete → 200
//   3. Duplicate → new file exists + force-disabled
//   4. Disable (PUT disabled:true) → manual-fire 409 → enable → manual-fire 200
//   5. Save-time: D69 (on_enter + forbidden) → 400; D70 (no triggers) → 400
//   6. Fire-time (4f.4 / D71): manual-fire missing input → 400 with structured
//      `missing`; pc_run_workflow missing input → 400 with same shape.
...
async function buildLifecycleApp(fixture: Fixture): Promise<LifecycleApp> {
  const { Hono } = await import('hono');
  const projectId = fixture.project.id as string;
  const workflowsDir = fixture.workflowsDir;
  const runtime = fixture.runtime;
  const registry = (runtime as unknown as { registry: InstanceType<typeof WorkflowRegistry> }).registry;

  const app = new Hono();

  // Mirror POST /workflows (create) — production: index.ts:1449
  app.post('/api/projects/:projectId/workflows', async (c) => {
    if (c.req.param('projectId') !== projectId) {
      return c.json({ ok: false, error: 'unknown project' }, 404);
    }
    ...
    if (!validation.ok || !validation.workflow) {
      return c.json({ ok: false, error: 'invalid workflow', errors: validation.errors }, 400);
    }
    const filePath = resolve(workflowsDir, `${wfId}.yaml`);
    if (existsSync(filePath)) {
      return c.json({ ok: false, error: `workflow already exists: ${wfId}` }, 409);
    }
    mkdirSync(workflowsDir, { recursive: true });
    writeFileSync(filePath, serializeWorkflow(validation.workflow), 'utf-8');
    registry.reload();
    return c.json({ ok: true, workflow: { id: wfId, fileName: `${wfId}.yaml`, path: filePath } }, 201);
  });

  // Mirror PUT /workflows/:wfId (edit) — production: index.ts:1503
  app.put('/api/projects/:projectId/workflows/:wfId', async (c) => {
    ...
    if (typeof payload.yamlText === 'string') {
      ...
      if (!parsed.ok || !parsed.workflow) {
        return c.json({ ok: false, error: 'invalid workflow', errors: parsed.errors }, 400);
      }
    ...
    if (typeof rawDef.id !== 'string' || rawDef.id !== wfId) {
      return c.json(
        { ok: false, error: `def.id must match URL workflow id (${wfId})` },
        400,
      );
    }
    const filePath = resolve(workflowsDir, `${wfId}.yaml`);
    ...
    writeFileSync(filePath, serializeWorkflow(validation.workflow), 'utf-8');
    registry.reload();
    return c.json({ ok: true, workflow: { id: wfId, fileName: `${wfId}.yaml`, path: filePath } });
  });

  // Mirror DELETE /workflows/:wfId — production: index.ts:1578
  app.delete('/api/projects/:projectId/workflows/:wfId', (c) => {
    ...
    const inFlight = runtime.inFlightRunsForWorkflow(wfId);
  ...
  // Mirror POST /workflows/:wfId/cancel-runs-and-delete — production: index.ts:1615
  app.post('/api/projects/:projectId/workflows/:wfId/cancel-runs-and-delete', async (c) => {
    ...
    for (const run of inFlight) {
      await runtime.cancelRunExternal(run.id, 'workflow deleted');
    }
    rmSync(filePath);
    registry.reload();
  ...
  // Mirror POST /workflows/:wfId/duplicate — production: index.ts:1655
  app.post('/api/projects/:projectId/workflows/:wfId/duplicate', async (c) => {
    if (c.req.param('projectId') !== projectId) {
      return c.json({ ok: false, error: 'unknown project' }, 404);
    }
    const wfId = c.req.param('wfId');
    const body = await c.req.json<{ newId?: string }>().catch(() => ({}) as { newId?: string });
    const srcPath = resolve(workflowsDir, `${wfId}.yaml`);
    ...
    if (!newId) {
      newId = `${wfId}-copy`;
      let n = 2;
      while (existsSync(resolve(workflowsDir, `${newId}.yaml`))) {
        newId = `${wfId}-copy-${n++}`;
      }
    }
    const newPath = resolve(workflowsDir, `${newId}.yaml`);
    ...
    if (!parsed.ok || !parsed.workflow) {
      return c.json({ ok: false, error: 'source workflow is invalid' }, 400);
    }
    ...
    if (!reValidation.ok || !reValidation.workflow) {
      return c.json({ ok: false, error: 'cloned failed validation' }, 400);
    }
    writeFileSync(newPath, serializeWorkflow(reValidation.workflow), 'utf-8');
    registry.reload();
    return c.json({ ok: true, workflow: { id: newId, fileName: `${newId}.yaml`, path: newPath } }, 201);
  });

  // Mirror POST /workflows/:wfId/fire (manual-fire) — production: index.ts:1740
  app.post('/api/projects/:projectId/workflows/:wfId/fire', async (c) => {
    ...
    try {
      const run = await runtime.fireManually({
        workflowId: wfId,
  ...
  // Mirror POST /workflow/run (pc_run_workflow MCP wrapper) — production: index.ts:2008
  app.post('/api/projects/:projectId/workflow/run', async (c) => {
    ...
    const inputs =
      body.input && typeof body.input === 'object' && !Array.isArray(body.input)
        ...
        : undefined;
    try {
      const run = await runtime.runWorkflow(name, inputs);
  ...
  return { app, projectId, workflowsDir, runtime };
}

test('4f.5 / D39 e2e smoke (HTTP): lifecycle arc — create → edit (def) → edit (yamlText) → duplicate → delete', async () => {
  const f = await mkFixture([]);
  try {
    const { app, projectId, workflowsDir } = await buildLifecycleApp(f);

    // 1. CREATE
    const createRes = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          def: {
            ...
            worktree: 'none',
            nodes: [{ id: 'stop', kind: 'cancel', cancel: 'done' }],
          },
        }),
      }),
    );
    assert.equal(createRes.status, 201);
    assert.ok(existsSync(resolve(workflowsDir, 'lifecycle-arc.yaml')));

    // 2. EDIT via def
    const editDefRes = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/lifecycle-arc`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          def: {
            id: 'lifecycle-arc',
            description: 'edited via def',
            triggers: { callable: true },
            worktree: 'none',
            nodes: [{ id: 'stop', kind: 'cancel', cancel: 'done' }],
          },
    ...
    assert.equal(editDefRes.status, 200);
    const afterDef = readFileSync(resolve(workflowsDir, 'lifecycle-arc.yaml'), 'utf-8');
    assert.match(afterDef, /description:.*edited via def/);

    // 3. EDIT via yamlText (PM raw-YAML tab)
    const yamlText = `id: lifecycle-arc
description: edited via yamlText
triggers:
  callable: true
worktree: none
nodes:
  - id: stop
    ...
    cancel: done
`;
    const editYamlRes = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/lifecycle-arc`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
    ...
    assert.equal(editYamlRes.status, 200);
    const afterYaml = readFileSync(resolve(workflowsDir, 'lifecycle-arc.yaml'), 'utf-8');
    assert.match(afterYaml, /description:.*edited via yamlText/);

    // 4. DUPLICATE — auto-named, force-disabled
    const dupRes = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/lifecycle-arc/duplicate`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({}),
      }),
    ...
    const dupBody = (await dupRes.json()) as { workflow: { id: string } };
    assert.equal(dupBody.workflow.id, 'lifecycle-arc-copy');
    const dupYaml = readFileSync(resolve(workflowsDir, 'lifecycle-arc-copy.yaml'), 'utf-8');
    assert.match(dupYaml, /disabled:\s*true/);

    // 5. DELETE original — no in-flight runs → 200
...
test('4f.5 / D39 e2e smoke (HTTP): delete with in-flight run → 409 → cancel-runs-and-delete → 200', async () => {
  const f = await mkFixture([]);
  try {
    const { app, projectId, workflowsDir, runtime } = await buildLifecycleApp(f);
    // Seed a workflow on disk + a synthetic in-flight run referencing it.
    const yaml = `id: delete-with-runs
triggers:
  callable: true
worktree: none
nodes:
...
`;
    writeFileSync(resolve(workflowsDir, 'delete-with-runs.yaml'), yaml, 'utf-8');
    (runtime as unknown as { registry: InstanceType<typeof WorkflowRegistry> }).registry.reload();
    const entry = (
      runtime as unknown as { registry: InstanceType<typeof WorkflowRegistry> }
    ).registry
      .snapshot()
      .valid.find((e) => e.workflow.id === 'delete-with-runs')!;
    // Create a run row stuck in pending so the in-flight guard triggers.
    const seededRun = runtime.createRun({
    ...
    // 1. Delete with in-flight → 409 + inFlightRunIds
    const blocked = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/delete-with-runs`, {
        method: 'DELETE',
      }),
    );
    assert.equal(blocked.status, 409);
    const blockedBody = (await blocked.json()) as { inFlightRunIds: string[] };
    assert.deepEqual(blockedBody.inFlightRunIds, [seededRun.id]);
    assert.ok(existsSync(resolve(workflowsDir, 'delete-with-runs.yaml')), 'file must survive 409');

    // 2. Cancel-runs-and-delete → 200 + cancelledRunIds
    const force = await app.fetch(
      new Request(
        `http://test.local/api/projects/${projectId}/workflows/delete-with-runs/cancel-runs-and-delete`,
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({}),
    ...
    const forceBody = (await force.json()) as { ok: boolean; cancelledRunIds: string[] };
    assert.equal(forceBody.ok, true);
    assert.deepEqual(forceBody.cancelledRunIds, [seededRun.id]);
    assert.ok(!existsSync(resolve(workflowsDir, 'delete-with-runs.yaml')));
    const cancelled = runtime.readRunForProject(seededRun.id);
...
test('4f.5 / D39 e2e smoke (HTTP): disable → manual-fire 409 → enable → manual-fire 200', async () => {
  const f = await mkFixture([]);
  try {
    const { app, projectId, workflowsDir, runtime } = await buildLifecycleApp(f);
    // Seed a manual-fireable workflow.
    writeFileSync(
      resolve(workflowsDir, 'toggle.yaml'),
      `id: toggle
triggers:
  callable: true
worktree: none
...
`,
    ...
    // 1. Disable via PUT def
    const disableRes = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/toggle`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          def: {
    ...
    // 2. Manual-fire → 409 disabled
    const blocked = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/toggle/fire`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({}),
      }),
    ...
    assert.match(blockedBody.error, /is disabled/);

    // 3. Enable via PUT def
    ...
    // 4. Manual-fire → 200 + runId
    const fired = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/toggle/fire`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({}),
    ...
    const firedBody = (await fired.json()) as { ok: boolean; runId: string };
    assert.equal(firedBody.ok, true);
    assert.ok(firedBody.runId);
    // Tick is deferred via setImmediate; give it a moment to land terminal.
    for (let i = 0; i < 25; i++) {
      const fresh = runtime.readRunForProject(firedBody.runId);
      if (fresh?.status === 'cancelled' || fresh?.status === 'complete') break;
      await new Promise((res) => setTimeout(res, 20));
    }
    const settled = runtime.readRunForProject(firedBody.runId);
    assert.equal(settled?.status, 'cancelled', 'cancel-only workflow should reach terminal');
  } finally {
    f.cleanup();
  }
});

test('4f.5 / D39 e2e smoke (HTTP): save-time D69 (on_enter + forbidden) and D70 (no triggers) reject with 400', async () => {
  const f = await mkFixture([]);
  try {
    const { app, projectId } = await buildLifecycleApp(f);

    // D69 cross-cut: on_enter + attached_to_work_item=forbidden → reject
    const d69 = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          def: {
            ...
            triggers: { on_enter: { stage_id: 'draft' } },
            worktree: 'none',
            nodes: [{ id: 'stop', kind: 'cancel', cancel: 'done' }],
          },
        }),
    ...
    assert.equal(d69.status, 400);
    const d69Body = (await d69.json()) as {
      error: string;
      errors: Array<{ path: string; message: string }>;
    };
    assert.match(d69Body.error, /invalid workflow/);
    assert.ok(
      d69Body.errors.some((e) => /on_enter/.test(e.message) || /attached/.test(e.message)),
      `expected on_enter+forbidden error in: ${JSON.stringify(d69Body.errors)}`,
    );

    // D70 savability: no triggers at all → reject
    const d70 = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          def: {
            id: 'd70-trigger-missing',
            triggers: {},
            worktree: 'none',
            nodes: [{ id: 'stop', kind: 'cancel', cancel: 'done' }],
          },
    ...
    assert.equal(d70.status, 400);
    const d70Body = (await d70.json()) as {
      error: string;
      errors: Array<{ path: string; message: string }>;
    };
    assert.ok(
      d70Body.errors.some((e) => /trigger/.test(e.message)),
      `expected trigger-required error in: ${JSON.stringify(d70Body.errors)}`,
    );
  } finally {
    f.cleanup();
  }
...
test('4f.5 / D39 e2e smoke (HTTP): fire-time D71 missing-inputs — manual-fire and pc_run_workflow both 400 with structured missing', async () => {
  const f = await mkFixture([]);
  try {
    const { app, projectId, workflowsDir, runtime } = await buildLifecycleApp(f);
    // Seed a workflow that declares an extra required input.
    writeFileSync(
      resolve(workflowsDir, 'needs-input.yaml'),
      `id: needs-input
triggers:
  callable: true
worktree: none
...
`,
    ...
    // 1. manual-fire missing → 400 + structured missing
    const manualMissing = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/needs-input/fire`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({}),
      }),
    );
    assert.equal(manualMissing.status, 400);
    const manualBody = (await manualMissing.json()) as { error: string; missing: string[] };
    assert.deepEqual(manualBody.missing, ['customerName']);
    assert.match(manualBody.error, /missing required input/);

    // 2. manual-fire supplied → 200
    const manualOk = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflows/needs-input/fire`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ inputs: { customerName: 'Acme Co' } }),
      }),
    );
    assert.equal(manualOk.status, 200);

    // 3. pc_run_workflow missing → 400 + structured missing
    const callableMissing = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflow/run`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name: 'needs-input', input: {} }),
      }),
    );
    assert.equal(callableMissing.status, 400);
    const callableBody = (await callableMissing.json()) as { error: string; missing: string[] };
    assert.deepEqual(callableBody.missing, ['customerName']);

    // 4. pc_run_workflow supplied → 200
    const callableOk = await app.fetch(
      new Request(`http://test.local/api/projects/${projectId}/workflow/run`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ name: 'needs-input', input: { customerName: 'Acme Co' } }),
      }),
    );
    assert.equal(callableOk.status, 200);
  } finally {
    f.cleanup();
  }
});
