...
1. **DB migration ledger lies — fresh-DB boot crash (HANDOFF to pod-work session)** — migrations `0031`/`0032` (run/pod `rev` stamps) are recorded *applied* in `__drizzle_migrations` but their columns were never added to `data/pc.sqlite`, so `runMigrations` skips them and boot crashes in the orchestrator-pod seed (`no such column: rev`). Patched the local dev DB by hand (added `workflow_runs_v2.rev`, `agent_runs.rev`, `agents.rev`; `projects.stages_rev` already present). **Root cause unfixed:** any fresh DB (teammate / dogfood / release) will crash-loop on first boot. The pod-work owner must find how the ledger got ahead of the schema (regenerated migration? restored DB?) and make 0031–0033 actually apply on a clean DB.
