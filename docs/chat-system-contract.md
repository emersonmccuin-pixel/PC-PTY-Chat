...
6. Add replay/backfill tests for refresh, reconnect, and session switch. Started; runtime snapshot, runtime-host route, WebSocket connect, and PTY handler tests now cover replay high-water refresh, reconnect queue state, new-session replay reset, resume replay high-water, connect envelope ordering, JSONL cursor/path persistence, and JSONL replay metadata broadcasts. Session-switch UI leakage coverage remains.
