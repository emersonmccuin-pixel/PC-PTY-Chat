...
**Cold-read order after `/clear`:** `MULTI-TENANCY-DESIGN.md` (still the source of truth for the multi-tenant contract; Session Q vendors the UI on top of it) → this doc (intro + `**Current session:**` line + the "Chassis — Multi-tenancy" section's Session Q checklist + the **second** Session Q mid-session log entry at the bottom, which covers Q8–Q10 and the Q11–Q13 plan). For Q11 specifically also skim v1's `apps/web/src/components/ProjectSettingsPanel.tsx` at `E:/Claude Code Projects/Personal/Project Companion/apps/web/src/components/ProjectSettingsPanel.tsx` (the vendor target — UI is API-shape-compatible with the trunk's per-project endpoints; no PTY-specific reshape needed). For workflow-runtime work: `DESIGN-WORKFLOWS-V2.md` is still the active design (node catalog + execution model + Decisions/Deferred). `DESIGN-WORKFLOWS-AND-CONTRACTS.md` and `PLANNING-CONTRACTS-MODELS.md` are superseded; skip on cold reads.
...
- [x] Q3. Vendor Shell (3-col layout, react-resizable-panels). Header + left ProjectRail + center workspace + right ActivityPanel (stub here, filled in Q12).
- [x] Q4. Vendor ProjectRail. Lists projects from `GET /api/projects`. Active-project state in zustand. "Create project" button surfaces the create-project modal.
- [x] Q5. Create-project flow: vendor FolderPicker + FolderBrowserModal. Wire to `GET /api/fs/browse` (drill into dirs) + `POST /api/fs/probe` (show the folder-state preview: "12 files, no .git — we'll init here and commit them as `Initial import`. Proceed?"). On confirm, `POST /api/projects` with mode derived from the probe.
- [x] Q6. Active-project plumbing: zustand store holds `activeProjectId`. WS hook filters events by `projectId === activeProjectId` (with an opt-in "all projects" mode for ActivityPanel per v1 §7). All API calls scope to the active project.
- [x] Q7. Vendor Tabs strip + WorkItems → KanbanBoard. Replaces N5's simple WorkItemsList. Live updates via WS. @dnd-kit/core for drag-between-stages. Retire `apps/web/src/components/work-items-list.tsx`. (work-items-list.tsx retired in Q3 commit `c235207` when the client.ts signatures broke its callsites.)
- [x] Q8. Vendor Orchestrator chat panel. WS-first reshape: drop v1's polling, subscribe to per-project chat events. Markdown via react-markdown + remark-gfm. Approval cards inline. Ask cards: stacked descriptions + Cancel button per Session M point 3. Bundled `<channel>` blocks: parse out and render one bubble per block (closes the Session M Followup).
- [x] Q9. Workflows tab (read-only). Vendor a minimal `WorkflowList` showing `<project>/.project-companion/workflows/*.yaml` from `GET /api/workflows`. Read-only YAML view + run history + pending-approval cards. Builder deferred to Session R.
...
Second mid-session checkpoint. Q8 / Q9 / Q10 shipped + committed (`42778ae`, `b3c4bb8`, `9cd86c2`). Q11–Q14 pending. No user test run yet — that gate still lives at the end of Q14. Tree clean, typecheck green across all 9 packages, `pnpm --filter @pc/web build` clean. User smoke-tested Q8–Q10 in the browser between sessions; nothing flagged.

**What works in the UI now** (boot: `pnpm dev` + `pnpm --filter @pc/web dev` → `http://127.0.0.1:5173/`):
1. Everything from the Q1–Q7 entry below, **plus:**
2. **Orchestrator tab (Q8):** WS event stream → bubbles. User (right-aligned), assistant (markdown via react-markdown + remark-gfm into `.markdown-body`), tool-line (suppression set parity with legacy), todos card, task-start / task-end with subagent pill, approval card (Approve / Reject with reason → `POST /api/projects/:id/approval/respond`), ask card (stacked options + descriptions + Cancel sending `__cancelled__` per Session M point 3). `<channel source="...">BODY</channel>` blocks inside a user-message turn split into one bubble per block (closes Followup at BUILDOUT.md:976). Composer (Enter to send, Shift+Enter newline) + Interrupt button via WS.
3. **Workflows tab (Q9):** three sections — Pending approvals (live cards), Workflows (registry: valid + invalid YAML files with per-error list), Recent runs (status pill + workflow id + run id tail + start/end). Refetches on workflow-relevant chat events.
4. **App settings (Q10):** gear icon in the header opens AppSettingsModal. Form fields: projectsFolder (via FolderBrowserModal), telemetryOptIn toggle, dataDir text input. Save → `PATCH /api/settings` with merged envelope. dataDir change triggers a dismissible restart-required banner above the Shell. CreateProjectModal now uses projectsFolder as the folder picker's initial path.

**Shape that landed:**
- `apps/web/src/components/Orchestrator.tsx` (new ~520 LOC). Vendor header references `apps/web/legacy/app.js` (the real reference renderer, since v1's `Orchestrator.tsx` is API-shape-incompatible). Sub-components for each bubble kind; `parseUserText` for the channel-block split.
- `apps/web/src/hooks/use-project-ws.ts` extended: typed `ChatEvent` union + `WsOutbound` union + `send(msg)` callback. Pure additive — existing Q6 consumers untouched.
- `apps/web/src/components/WorkflowList.tsx` (new ~340 LOC). Three-section pane; lazy refetch on chat events.
- `apps/web/src/components/AppSettingsModal.tsx` (new ~170 LOC). Mirrors `CreateProjectModal.tsx`'s overlay shape; reuses `FolderBrowserModal`.
- **Domain envelope:** `packages/domain/src/settings.ts` grew `projectsFolder` (default `~/Projects/`) and `activityPanel.{open,showAllProjects}`. `defaultGlobalSettings(dataDir, homeDir)` is now 2-arg; `withSettingsDefaults(stored, dataDir, homeDir)` backfills missing fields on read so no schema migration is needed. `packages/db/src/migrate.ts` updated; `packages/db/src/repos/settings.ts` unchanged (still JSON column).
- **Server adds (3 endpoints):** `GET /api/settings`, `PATCH /api/settings` (partial merge + `restartRequired` flag), `GET /api/projects/:id/workflow-runs` (backed by new `listRunsByProject(projectId)` repo helper that uses the existing `workflow_runs_project_idx` index).
- **Workflow-runtime add:** `readRunsForProject()` thin wrapper.
- **API client:** added `getSettings()`, `patchSettings()`, internal `postJsonMethod` helper (so PATCH could share the same JSON-or-throw error path as POST).

**Findings worth carrying forward:**
- The LSP cache inside the harness frequently goes stale after edits — diagnostics complaining about "declared but never used" or "Property X does not exist" often clear after a real `pnpm -r typecheck`. Trust the typecheck output over the diagnostic stream when they disagree.
- `c.req.json<Partial<T>>().catch(() => ({}))` widens the inferred type to `Partial<T> | {}` and `{}` then loses every key. Use `.catch((): Partial<T> => ({}))` to keep the type honest. Came up in `PATCH /api/settings`.
- `withSettingsDefaults` on read is the right shape for additive envelope changes: zero migration cost, old rows just work, new fields get the default until the user PATCHes. Worth reusing when other settings categories grow (e.g. UI prefs, future telemetry buckets).
- The trunk has **no dedicated chat-events.changed envelope**; WorkflowList's lazy refetch piggy-backs on `approval-required` / `task-start` / `task-end` / `work-items-changed`. Works fine; flagged here in case Q12 wants a tighter signal.
- `prose prose-invert prose-sm` Tailwind utilities require `@tailwindcss/typography` (not installed). The pre-existing `.markdown-body` global stylesheet (in `index.css`) is the right hook for assistant markdown — already styled for Vellum, supports gfm tables, code blocks, blockquotes.

**Open Q11–Q14 (in BUILDOUT order):**
- **Q11 — ProjectSettingsPanel.** Largest of the remaining three. Every backing endpoint already exists on the trunk: `PATCH /api/projects/:id` (rename + git_remote), `GET/POST /api/agents` (library), `GET /api/projects/:id/agents` + `POST /api/projects/:id/agents` (per-project copy from library) + `PATCH /api/projects/:id/agents/:name` (edit copy in place), `DELETE /api/projects/:id` (soft-delete), `DELETE /api/projects/:id/files` (danger zone). v1's `ProjectSettingsPanel.tsx` is the vendor target — API-shape-compatible, no PTY-specific reshape needed. The "save edited project agent back to library" affordance is `POST /api/agents` with the project copy's body. Replace the `<Stub label="Project settings" milestone="Q11" />` in `Shell.tsx`.
- **Q12 — ActivityPanel.** Two pieces: full event log replacing today's stub list, and the "all projects" toggle. The toggle reads `settings.activityPanel.showAllProjects` (already in the Q10 envelope) and `settings.activityPanel.open` (replaces App.tsx's local `useState(true)`; PATCH on toggle). Multi-project subscribe needs a separate hook (one WS per project) — `useProjectWs` is single-connection by design; build `useAllProjectsWs` alongside, opening N parallel sockets. The trunk server is per-project WS-scoped (`/ws?projectId=ULID`, opening a second supersedes the first), so the multi-project hook needs one socket per project simultaneously.
- **Q13 — WS client hardening.** Exponential backoff 2 → 5 → 15 → 30s cap, single banner per disconnect, event-timestamp dedup vs server replay. The legacy `apps/web/legacy/app.js` lines 495–552 has the exact algorithm — port into `hooks/use-project-ws.ts`. Today the hook just marks `status: 'closed'` on socket close with no retry. Server replays `events.jsonl` on every WS connect (per `apps/server/src/index.ts:762`), so dedup needs to fire there too once reconnects start working.
- **Q14 — typecheck + build green.** Already green; just keep it that way.

**Cold-read recovery for the next session (resuming Session Q):** read `MULTI-TENANCY-DESIGN.md` § "Per-project filesystem layout" + § "ProjectRuntime abstraction" → this second Session Q mid-session entry → the Session Q checklist above. For Q11 specifically also skim v1's `apps/web/src/components/ProjectSettingsPanel.tsx` (read-only path `E:/Claude Code Projects/Personal/Project Companion/apps/web/src/components/ProjectSettingsPanel.tsx`) and the trunk's `PATCH /api/projects/:projectId` + `/api/projects/:projectId/agents*` endpoints in `apps/server/src/index.ts:208-385`.

### Session Q (mid-session) — UI vendor + multi-tenant shell, milestones Q1–Q7 (2026-05-17)

Mid-session checkpoint. Q1–Q7 shipped + committed; Q8–Q14 pending. No user test run yet — that gate lives at the end of Q14. Tree clean, typecheck green across all 9 packages, `pnpm --filter @pc/web build` clean.

**What works in the UI today** (boot: `pnpm dev` + `pnpm --filter @pc/web dev` → `http://127.0.0.1:5173/`):
1. Empty rail shows "No projects yet."
2. "+ New project" → modal: name input, Browse… folder picker with `GET /api/fs/browse` (defaults to `~/`), drill into dirs / select.
3. After folder pick: `POST /api/fs/probe` runs — preview reads "Empty folder — will git init here" OR "N entries, no .git — will commit as Initial import then add scaffold" OR refuses on existing `.git`.
4. Create → `POST /api/projects` with mode derived from probe. Project lands in rail + becomes active.
5. Switch projects via rail — workspace re-keys: kanban + activity panel + persisted tab.
6. Kanban (Work items tab): @dnd-kit DnD between stages, add-card form per column, optimistic + reconcile via WS `work-items-changed` (server emits this on every mutation, P14 already tagged `projectId`).
7. Activity panel (right): WS status pill (idle/connecting/live/closed), running event-type log. ProjectRail "+ New project" sits in the rail footer. Header toggles activity panel.
8. Orchestrator / Workflows / Project settings tabs render `<Stub label=… milestone=Qx />` placeholders.

**Shape that landed:**
- Vellum theme + JetBrains Mono vendored into `apps/web/src/index.css`; first vendored file in the repo, so `THIRD_PARTY.md` lives at the root now. Vendor headers per file (`apps/web/src/components/{Shell,ProjectRail,KanbanBoard,Tabs,FolderBrowserModal}.tsx` + `apps/web/src/index.css`).
- API client (`apps/web/src/api/client.ts`) replaced wholesale: types match `@pc/domain` (slug / folderPath / gitRemote), all per-project endpoints take ULID, plus `listProjects` / `createProject` / `browseFolder` / `probeFolder`. Singleton `/api/project` / `/api/work-items` helpers are gone.
- Zustand stores: `store/active-project.ts` (slug + persist) and `store/per-project-tab.ts` (`Record<slug, Tab>` + persist). Both use `zustand/middleware`'s `persist`.
- WS client lives in `hooks/use-project-ws.ts`. Returns `{ events, status, clear }`. Single connection per active project; switching projects closes + reopens. Q13 will harden reconnect/backoff/dedup.
- Shell uses **react-resizable-panels v4** — distinct API from v1 (`Group` / `Panel` / `Separator` + `usePanelRef`); v3's `PanelGroup` / `PanelResizeHandle` / `ImperativePanelHandle` no longer exist. v1's `autoSaveId` is now `useDefaultLayout` — we haven't wired persistence yet (acceptable, defaults are fine).
- Server: 3 small adds to broadcast on work-item mutations (`{ type: 'work-items-changed', change, workItem }` in `apps/server/src/index.ts`). Only the HTTP path broadcasts — MCP-tool-driven mutations route through the same runtime methods but the broadcast is wired one level up; **followup** if `pc_move_work_item` needs to surface in the UI without a refetch.
- `apps/web/src/components/work-items-list.tsx` deleted in Q3 when the new client.ts signatures broke its callsites (Q7 was going to retire it anyway).

**Findings worth carrying forward:**
- `react-resizable-panels` v4 is a hard API break from v3. If anyone reads v1's `apps/web/` for guidance, their PanelGroup/PanelResizeHandle/ref usage will not compile. The migration is `PanelGroup` → `Group`, `PanelResizeHandle` → `Separator`, `ref={ref}` → `panelRef={usePanelRef()}`, `id` instead of `autoSaveId`.
- Tailwind v4 `@theme` block needs `--font-*` and `--color-*` tokens registered explicitly to flow into utility classes. The Vellum import path needs to be **before** `@import 'tailwindcss'` so the fontsource CSS lands first (otherwise Tailwind's reset fights it).
- `verbatimModuleSyntax: true` + `moduleResolution: 'bundler'` is the existing tsconfig — type-only imports must use `import type`. All new components comply.
- The trunk server is per-project WS-scoped (`/ws?projectId=ULID`, opening a second supersedes the first). v1's "filter events by activeProjectId" pattern is therefore enforced at connection time, not the message layer. The "all projects" ActivityPanel toggle (Session Q12) will need a separate hook that opens N parallel connections.
- Heredoc gotcha on Windows/bash interop: backticks inside unquoted commit-message segments hit command substitution. Q7's commit lost the `` `pc.tabs` `` token to a "command not found" warning; the commit landed but the substring is gone. Use HEREDOC with `<<'EOF'` (single-quoted delimiter) for any commit message with backticks or `$`. Already followed elsewhere; flagged here for next-session muscle memory.

**Open Q8–Q14 (in BUILDOUT order):**
- **Q8 — Orchestrator chat panel.** Biggest remaining unit. The plan to vendor v1's `Orchestrator.tsx` (1295 LOC) does **not** apply: v1 is API-message-shaped (sessions / messages / providers / SSE), the trunk is PTY-event-shaped (WS stream of `raw`/`state`/`turn-end`/`event`/`exit`/`ask`/`channel-event`). Use `apps/web/legacy/app.js` (~1030 LOC, vanilla) as the reference renderer instead — it has the existing event→bubble protocol (`user`/`assistant`/`tool-start`/`tool-end`/`todos`/`task-start`/`task-end`/`approval-required` + `ask` cards). Needed extensions over the legacy: react-markdown + remark-gfm for assistant text; bundled `<channel>` block parser (Session M Followup) — split assistant text into one bubble per block; Ask cards stacked descriptions + Cancel button per Session M point 3 (legacy already has both, port as-is). Approval responses go via `POST /api/projects/:projectId/approval/respond` (already wired server-side). Composer + interrupt go via WS — need to extend `useProjectWs` to expose a `send(msg)`. Reverted the prep-only `send` addition during the wrap to keep the tree clean — Q8 should land it as part of one cohesive commit.
- **Q9 — Workflows tab (read-only).** Vendor a minimal `WorkflowList` reading `GET /api/projects/:projectId/workflows` — that endpoint already exists and returns `{ valid, invalid }`. Run history + pending approvals: there's `GET /api/projects/:projectId/approvals` for the latter; for runs, no list endpoint today (only run-trigger + node-complete/node-failed). Probably need a new `GET /api/projects/:projectId/workflow-runs` server endpoint or surface runs via WS history — flag for the Q9 design notes.
- **Q10 — AppSettingsModal.** `GlobalSettings` in `packages/domain/src/settings.ts` only has `dataDir` + `telemetryOptIn` today. The Session N close-out planned an envelope with `projectsFolder` + `activityPanel.{open,showAllProjects}` (the latter consumed by Q12). Q10 has to ship that envelope and the server endpoints (`GET /api/settings`, `PATCH /api/settings`). v1 has `SettingsEnvelope` + `RestartRequiredBanner` patterns to vendor.
- **Q11 — ProjectSettingsPanel.** PATCH /api/projects/:id (exists), agent library picker uses GET/POST /api/agents + POST /api/projects/:id/agents (all exist), soft-delete uses DELETE /api/projects/:id (exists), danger-zone file removal uses DELETE /api/projects/:id/files (exists). v1's `ProjectSettingsPanel.tsx` is the vendor target — the trunk has the endpoints to back it.
- **Q12 — ActivityPanel.** Two pieces: full event log replacing today's stub list, and the "all projects" toggle. The toggle needs the Q10 settings envelope landed first. Multi-project subscribe needs a separate hook (one WS per project) — see "Findings" above.
- **Q13 — WS client hardening.** Exponential backoff 2 → 5 → 15 → 30s cap, single banner per disconnect, event-timestamp dedup vs server replay. The legacy `apps/web/legacy/app.js` lines 495–552 has the exact algorithm — port into `hooks/use-project-ws.ts`.
- **Q14 — typecheck + build green.** Already green today (just keep it that way through Q8–Q13).

**Cold-read recovery for the next session (resuming Session Q):** read `MULTI-TENANCY-DESIGN.md` § "Per-project filesystem layout" + § "ProjectRuntime abstraction" + § "Channel server" → this Session Q mid-session entry → the Session Q checklist above. For Q8 specifically also skim `apps/web/legacy/app.js` lines 1–550 (the event→bubble renderer + WS connect/reconnect block).
