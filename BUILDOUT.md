...
**Current session:** Session Q **in flight** — UI vendor + multi-tenant shell. **Q1–Q7 of 14 shipped + ticked + committed** (deps, Vellum re-skin, 3-col Shell, ProjectRail + active-project store, create-project flow w/ folder picker + probe, per-project WS hook, Tabs + KanbanBoard + WS work-item broadcasts). Q8–Q14 pending; no user test run yet (the test gate sits at the end of Q14). Tree clean, `pnpm -r typecheck` green, `pnpm --filter @pc/web build` clean. **Resume at Q8 — Orchestrator chat panel** — and note that v1's `Orchestrator.tsx` is API-message-shaped and does **not** vendor over to this PTY-event-shaped trunk; use `apps/web/legacy/app.js` as the renderer reference instead. Full Q1–Q7 outcomes + Q8–Q14 plan + cold-read order are in the Session Q mid-session log entry at the bottom of this doc.

**Rig lives at:** `E:/Projects/Caisson/`. All paths in this doc are relative to that root.

**Cold-read order after `/clear`:** `MULTI-TENANCY-DESIGN.md` (still the source of truth for the multi-tenant contract; Session Q vendors the UI on top of it) → this doc (intro + `**Current session:**` line + the "Chassis — Multi-tenancy" section's Session Q checklist + the Session Q mid-session log entry at the bottom). For Q8 specifically also skim `apps/web/legacy/app.js` lines 1–550 (the vanilla event→bubble renderer + WS reconnect block — the real reference for the chat panel reshape, since v1's `Orchestrator.tsx` doesn't apply to PTY mode). For workflow-runtime work: `DESIGN-WORKFLOWS-V2.md` is still the active design (node catalog + execution model + Decisions/Deferred). `DESIGN-WORKFLOWS-AND-CONTRACTS.md` and `PLANNING-CONTRACTS-MODELS.md` are superseded; skip on cold reads.
