...
test('inside Electron → app binary + ELECTRON_RUN_AS_NODE', () => {
  const launcher = resolveNodeLauncher({}, 'C:/Apps/PC/ProjectCompanion.exe', true);
  assert.equal(launcher.command, 'C:/Apps/PC/ProjectCompanion.exe');
...
test('PC_NODE_LAUNCHER override wins over Electron detection', () => {
  const launcher = resolveNodeLauncher(
    ...
    'C:/Apps/PC/ProjectCompanion.exe',
    true,
  );
