...
- Slice 002 build code is committed in Session 12, but final implemented status waits for Session 13 verify/close.
...
- Session 13 must review and verify slice 002 only before any slice 003 planning starts.
...
| Project live outbox build-slice plan | planned | `refactor plan/build-slices/002-project-live-outbox.md` | Live events/server/web/DB/projects | Slice 001, live events and outbox spec, definitive session pathway | Created in manual Session 11 and built in manual Session 12. Scope stayed durable `live_outbox` for `project.changed` only: canonical live-event contracts, additive DB migration/repo, atomic project mutation/outbox publisher boundary, replay route, websocket dual fanout, web cursor/refetch behavior, tests, rollback, and stop conditions. Final `implemented` status is reserved for Session 13 verify/close. |
