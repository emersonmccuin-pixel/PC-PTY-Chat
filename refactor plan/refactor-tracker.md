...
- Manual Session 11 is complete.
- Slice 001 is fully verified and has no remaining slice-001 blockers.
- Slice 002 is planned at `refactor plan/build-slices/002-project-live-outbox.md`.
- The next unchecked manual row is Session 12 in `refactor plan/refactor-session-tracker.md`.
- The required session order is defined in `refactor plan/definitive-session-pathway.md`.
- Session 12 must build only `refactor plan/build-slices/002-project-live-outbox.md`.
...
| Project live outbox build-slice plan | planned | `refactor plan/build-slices/002-project-live-outbox.md` | Live events/server/web/DB/projects | Slice 001, live events and outbox spec, definitive session pathway | Created in manual Session 11. Scope is durable `live_outbox` for `project.changed` only: canonical live-event contracts, additive DB migration/repo, project mutation publisher boundary, replay route, websocket dual fanout, web cursor/refetch behavior, tests, rollback, and stop conditions. No implementation code changed. |
