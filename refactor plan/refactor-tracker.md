...
- Remaining open gap: create/delete browser verification is blocked in the current running dev environment because `POST /api/projects` fails at `git init -b main` before a `project.changed` event can be emitted.
